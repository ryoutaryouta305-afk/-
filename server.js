...
const RULE_LABELS = {
  ...
  ambulance: '救急車(9+9)',
  shakaHoonko: '報恩講・降誕会',
  nightingale: 'ナイチンゲール(A+Q)',
  miyakoOchi: '都落ち',
  cardExchange: 'カード交換',
  jokerFoul: 'ジョーカー上がり反則',
};
...
function createRoom(hostSocketId, hostName) {
  ...
  const room = {
    ...
    spectators: [], // {id, name} 途中参加してきた観戦者。次のラウンドから正式参加になる
  };
...
const FREE_COMBOS = [
  ...
  { kind: 'nightingale', ruleKey: 'nightingale', ranks: [12, 14] },
...
function checkFinish(room, player) {
  if (player.hand.length === 0 && !player.finished) {
    ...
    if (player.pendingFoulFinish) {
      player.foulFinish = true;
      player.pendingFoulFinish = false;
    }
  }
}

// ジョーカーを使い切って手札が0になった場合、上がりを反則(強制最下位)として記録しておく
// 対象は通常プレイの単騎(1枚)・ペア(2枚)のみ。AK等の必殺コンボでジョーカーをワイルドとして
// 使った場合は対象外(反則にはならない)。
function noteJokerFinish(room, player, cards) {
  if (!room.rules.jokerFoul) return;
  if (player.finished) return;
  if (cards.length > 2) return;
  if (player.hand.length === 0 && cards.some(c => c.rank === 99)) {
    player.pendingFoulFinish = true;
  }
}

// 反則上がり(ジョーカー上がり)をしたプレイヤーを、この対戦の最下位グループに強制的に落とす
function reorderForJokerFoul(room) {
  const foulIds = room.finishOrder.filter(pid => {
    const p = room.players.find(pp => pp.id === pid);
    return p && p.foulFinish;
  });
  if (foulIds.length === 0) return;
  const normalIds = room.finishOrder.filter(pid => !foulIds.includes(pid));
  room.finishOrder = [...normalIds, ...foulIds];
  foulIds.forEach(pid => {
    const p = room.players.find(pp => pp.id === pid);
    pushLog(room, `${p ? p.name : '?'} はジョーカー上がりの反則！強制的に最下位グループに`);
  });
  triggerEffect(room, 'jokerFoul', {});
}

// 都落ち: 大富豪以外の人が新たに大富豪(1位)になった瞬間、前回の大富豪は強制的に大貧民(最下位)になる
function applyMiyakoOchi(room) {
  if (!room.rules.miyakoOchi || !room.previousPlacements) return;
  const prevChamp = room.previousPlacements.find(p => p.title === '大富豪');
  if (!prevChamp) return;
  const n = room.players.length;
  if (room.finishOrder.length !== n) return;

  const newChampId = room.finishOrder[0];
  if (newChampId === prevChamp.playerId) return; // 大富豪防衛 → 都落ちなし

  const idx = room.finishOrder.indexOf(prevChamp.playerId);
  if (idx < 0 || idx === room.finishOrder.length - 1) return; // 既に最下位なら何もしない

  room.finishOrder.splice(idx, 1);
  room.finishOrder.push(prevChamp.playerId);
  const prevChampPlayer = room.players.find(p => p.id === prevChamp.playerId);
  pushLog(room, `都落ち発生！${prevChampPlayer ? prevChampPlayer.name : '前大富豪'} が大貧民に転落しました`);
  triggerEffect(room, 'miyakoOchi', { label: '都落ち', by: prevChampPlayer ? prevChampPlayer.name : '' });
}
...
function finalizeAfterPlay(room, player, includesEight, turnOverride, clearKind) {
  ...
  if (turnOverride !== undefined && turnOverride !== null) {
    const target = room.players[turnOverride];
    // 上がった(手札0になった)本人の手番を強制すると進行不能になるため、
    // その場合は次の生存プレイヤーへ回す
    room.turnIndex = (target && target.finished) ? nextActiveIndex(room, turnOverride) : turnOverride;
    return;
  }
  if (includesEight) {
...
function serializeRoom(room, forSocketId) {
  ...
  const isSpectator = room.spectators.some(s => s.id === forSocketId);
  // 観戦者、または自分がすでに上がって(finished)待機中の場合は、
  // ゲームを追う楽しみのため全員の手札をオープンにする
  const canSeeAllHands = isSpectator || (me && me.finished);
  ...
  return {
    ...
    players: room.players.map(p => ({
      ...
      hand: (p.id === forSocketId || canSeeAllHands) ? p.hand : undefined,
    })),
    spectators: room.spectators.map(s => ({ id: s.id, name: s.name, isYou: s.id === forSocketId })),
...
function broadcastRoom(room) {
  ...
  for (const s of room.spectators) {
    io.to(s.id).emit('room-state', serializeRoom(room, s.id));
  }
}
...
function commitNormalPlay(room, player, cards, play) {
  ...
  noteJokerFinish(room, player, cards);
...
function handleBonusAfterGroupPlay(room, player, play, cards) {
  ...
  if (rules.bomberQ && play.rank === 12) {
    triggerEffect(room, 'bomberQ', { label: '12ボンバー', by: player.name });
    room.pendingAction = {
      kind: 'declareRank',
      playerId: player.id,
      count: play.count,
      reason: `12ボンバー：捨てさせるランクを${play.count}種類選んでください`,
    };
    return true;
  }
  if (rules.sevenGive && play.rank === 7) {
    ...
    triggerEffect(room, 'sevenGive', { label: '7渡し', by: player.name });
  ...
  if (rules.tenDiscard && play.rank === 10) {
    if (player.hand.length === 0) return false;
    const count = Math.min(play.count, player.hand.length);
    triggerEffect(room, 'tenDiscard', { label: '10捨て', by: player.name });
  ...
  if (rules.jBack && play.rank === 11) {
    ...
    triggerEffect(room, 'jBack', { label: 'Jバック', by: player.name });
    return false;
  }
  if (rules.ambulance && play.rank === 9 && play.count === 2) {
    ...
    triggerEffect(room, 'ambulance', { label: '救急車', by: player.name });
...
function handleFreeCombo(room, player, combo, cards) {
  ...
  triggerEffect(room, combo.kind, { label: comboName(combo.kind), by: player.name });
  ...
  if (combo.kind === 'nightingale') {
    ...
    if (player.hand.length === 0 || room.discardPile.length === 0) {
      // 手札が尽きた、または墓地が空の場合も「自分の手番」強制は適用される
      finalizeAfterPlay(room, player, false, selfIdx);
      return;
    }
    room.pendingAction = { kind: 'takeGraveyard', playerId: player.id, reason: 'ナイチンゲール：墓地から1枚回収できます', afterTurnOverride: selfIdx };
    return;
  }
...
function comboName(kind) {
  return { ak: 'AK', nightingale: 'ナイチンゲール', shaka: '降誕会', houonko: '報恩講' }[kind] || kind;
}
...
io.on('connection', socket => {
  ...
  socket.on('join-room', ({ name, roomId }) => {
    ...
    const cleanName = (name || '').slice(0, 10) || 'プレイヤー';

    if (room.phase !== 'waiting') {
      // ゲーム進行中は観戦者として参加し、今の試合が終わったら正式に参加できる
      room.spectators.push({ id: socket.id, name: cleanName });
      socket.join(room.id);
      socket.data.roomId = room.id;
      socket.data.spectator = true;
      pushLog(room, `${cleanName} が観戦者として参加しました(次のラウンドから参加できます)`);
      broadcastRoom(room);
      return;
    }

    if (room.players.length >= 6) { socket.emit('error-message', '満員です(6人まで)'); return; }
    addPlayer(room, socket.id, cleanName);
    socket.join(room.id);
    socket.data.roomId = room.id;
    pushLog(room, `${cleanName} が参加しました`);
    broadcastRoom(room);
  });
  ...
  socket.on('next-round', () => {
    ...
    // 観戦者を正式なプレイヤーに昇格(6人まで)
    while (room.spectators.length > 0 && room.players.length < 6) {
      const s = room.spectators.shift();
      addPlayer(room, s.id, s.name);
      const promoted = room.players[room.players.length - 1];
      const sock = io.sockets.sockets.get(s.id);
      if (sock) sock.data.spectator = false;
      pushLog(room, `${promoted.name} が次のラウンドから参加します`);
    }
  ...
  socket.on('resolve-action', (payload = {}) => {
    ...
    if (pending.kind === 'declareRank') {
      const need = pending.count || 1;
      const requested = Array.isArray(payload.ranks) ? payload.ranks : (payload.rank !== undefined ? [payload.rank] : []);
      const ranks = [...new Set(requested)].filter(r => DECLARABLE_RANKS.includes(r)).slice(0, need);
      if (ranks.length === 0) return;
      let removedTotal = 0;
      room.players.forEach(p => {
        const removed = p.hand.filter(c => ranks.includes(c.rank));
        if (removed.length) {
          p.hand = p.hand.filter(c => !ranks.includes(c.rank));
          room.discardPile.push(...removed);
          removedTotal += removed.length;
        }
      });
      const rankLabels = ranks.map(rankLabel).join('・');
      pushLog(room, `${player.name} が「${rankLabels}」を宣言！全員がそのランクを捨てました(${removedTotal}枚)`);
      room.pendingAction = null;
      const ctx = room.postPlayContext || {};
      room.postPlayContext = null;
    ...
    if (pending.kind === 'exchangeGive') {
      ...
      triggerEffect(room, 'cardExchange', { from: player.name, to: receiver.name, count: cardIds.length });
      room.pendingAction = null;
  ...
  socket.on('send-chat', ({ text }) => {
    ...
    // チャットだけならゲーム画面全体を再描画させる必要はないので、
    // 軽量な専用イベントで配信する(broadcastRoomは使わない)
...
function startCardExchange(room) {
  ...
  function forcedGive(loserTitle, winnerTitle, count) {
    ...
    triggerEffect(room, 'cardExchange', { from: loser.name, to: winner.name, count: takeCount });
