...
function detectFreeCombo(cards, rules) {
  const ranks = rankSetOf(cards);
  if (rules.ak && cards.length === 2 && sameSet(ranks, [13, 14])) {
    return { kind: 'ak' };
  }
  if (rules.nightingale && cards.length === 2 && sameSet(ranks, [9, 14])) {
    return { kind: 'nightingale' };
  }
  if (rules.shakaHoonko && cards.length === 3 && sameSet(ranks, [5, 14, 15])) {
    return { kind: 'shaka' };
  }
  if (rules.shakaHoonko && cards.length === 3 && sameSet(ranks, [6, 14, 14])) {
    return { kind: 'houonko' };
  }
  return null;
}
...
function finalizeAfterPlay(room, player, includesEight, turnOverride) {
  ...
  if (includesEight) {
    clearField(room, `場が流れ、${player.name} が続けて出せます`);
...
io.on('connection', socket => {
  ...
  socket.on('join-room', ({ name, roomId }) => {
    ...
    if (room.players.length >= 5) { socket.emit('error-message', '満員です(5人まで)'); return; }
  ...
  socket.on('start-game', () => {
    ...
    if (room.players.length < 2 || room.players.length > 5) return;
    startGame(room);
    broadcastRoom(room);
  });
  ...
  socket.on('play-cards', ({ cardIds }) => {
    ...
    if (room.field.count === 0) {
      const combo = detectFreeCombo(cards, room.rules);
      if (combo) {
        player.hand = player.hand.filter(c => !cardIds.includes(c.id));
        handleFreeCombo(room, player, combo, cards);
        broadcastRoom(room);
        return;
      }
    }
    ...
    if (bonusPending) {
      room.postPlayContext = { playerId: player.id, includesEight: clearsField };
      checkFinish(room, player);
    ...
    finalizeAfterPlay(room, player, clearsField);
    broadcastRoom(room);
  });
  ...
  socket.on('resolve-action', (payload = {}) => {
    ...
    if (pending.kind === 'give') {
      ...
      room.postPlayContext = null;
      finalizeAfterPlay(room, player, !!ctx.includesEight, pending.afterTurnOverride);
      broadcastRoom(room);
      return;
    }
    ...
    if (pending.kind === 'discard') {
      ...
      const ctx = room.postPlayContext || {};
      room.postPlayContext = null;
      finalizeAfterPlay(room, player, !!ctx.includesEight);
      broadcastRoom(room);
      return;
    }
    ...
    if (pending.kind === 'chooseCard') {
      ...
      const ctx = room.postPlayContext || {};
      room.postPlayContext = null;
      finalizeAfterPlay(room, player, !!ctx.includesEight, targetIdx);
      broadcastRoom(room);
      return;
    }
...
function handleDisconnect(socket) {
  ...
  if (!stillConnected) { rooms.delete(roomId); return; }
  broadcastRoom(room);
}
