...
function renderGame(room) {
  ...
  if (me && me.hand) {
    ...
    // 画面サイズに関わらず横スクロールなしできっちり収まるよう、実際のカード幅と
    // 利用可能な横幅から重なり量を逆算する(スマホ縦画面でも扇がちゃんと見える)
    const cardWidth = window.innerWidth <= 480 ? 60 : 74;
    const availWidth = (handWrap.clientWidth || window.innerWidth * 0.94) - 4;
    let overlap = 0;
    if (n > 1) {
      const rawOverlap = ((n * cardWidth) - availWidth) / (n - 1);
      overlap = Math.max(0, Math.min(cardWidth * 0.84, rawOverlap));
    }
