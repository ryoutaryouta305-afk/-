...
function buildCardEl(card, { onClick, selected, tilt, lift, dealDelay } = {}) {
  ...
  if (card.rank === 99) {
    ...
    // 絵札(J/Q/K): 金の縁取りフレーム + 文字
    const face = document.createElement('div');
    face.className = 'face-royal';
    face.innerHTML = `<span class="royal-letter">${cardLabel(card)}</span><span class="royal-suit">${SUIT_MARK[card.suit]}</span>`;
    div.appendChild(face);
  } else {
...
function renderGame(room) {
  ...
  oppWrap.innerHTML = '';
  const opponents = room.players.filter(p => !p.isYou);
