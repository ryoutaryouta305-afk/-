...
function renderGame(room) {
  ...
  if (me && me.hand) {
    ...
    // カードが小さくなりすぎないよう、重なりには上限を設ける。
    // 手札が多くて収まりきらない場合は横スクロール(選択時に自動で中央へ)で対応する。
    const cardWidth = window.innerWidth <= 480 ? 72 : 84;
    const availWidth = (handWrap.clientWidth || window.innerWidth * 0.94) - 4;
    let overlap = 0;
    if (n > 1) {
      const rawOverlap = ((n * cardWidth) - availWidth) / (n - 1);
      overlap = Math.max(0, Math.min(cardWidth * 0.6, rawOverlap));
    }
    me.hand.forEach((card, i) => {
      const el = buildCardEl(card, {
        selected: selectedIds.has(card.id),
        onClick: myNormalTurn ? () => {
          const justSelected = toggleSelect(card.id);
          renderGame(currentRoom);
          if (justSelected) {
            const el2 = handWrap.querySelector(`[data-id="${card.id}"]`);
            if (el2 && el2.scrollIntoView) el2.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
          }
        } : null,
...
function renderPendingPanel(room, me) {
  ...
  if (p.kind === 'give' || p.kind === 'discard') {
    ...
    me.hand.forEach((card, i) => {
      ...
      if (i > 0) el.style.marginLeft = '-28px';
      list.appendChild(el);
    });
    panel.appendChild(list);
